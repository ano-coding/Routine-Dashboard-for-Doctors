  ...
  return (
    ...
    >
      ...
      <div className="flex  select-none flex-wrap gap-3">
        {FILTERS.map((filter) => (
          <div
            className="flex items-center  justify-center gap-1 rounded-lg border-[1.8px] border-solid border-darkslategray-light px-4 py-2.5"
            key={filter.label}
          >
            <img className="h-3.5 w-full" src={`${filter.icon}.svg`} />
            <span className="whitespace-nowrap  font-dm-sans text-[14px] font-medium tracking-[-0.02em] text-darkslategray-200">
              {filter.label}
            </span>
          </div>
        ))}
      ...
      <section className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        {ROUTINE.map((routine) => (
          ...
          >
            <img
              src={`/${routine.image}`}
              className=" h-[12.1rem] w-32 rounded-lg object-cover lg:w-[12.1rem]"
            />
            <div className="flex flex-1 flex-col  ">
              ...
              <div className="w-full xl:w-3/4">
                <button className="w-full flex-1 whitespace-nowrap rounded-lg border-[1.5px] border-solid border-darkOliveGreen-dark  py-[12px] text-center font-nunito text-base font-semibold leading-[20px] tracking-[-0.01em] text-darkOliveGreen-dark">
                  Edit Routine
                </button>
              </div>
            </div>
      ...
      <div className="mb-5 flex flex-col gap-10 overflow-hidden rounded-xl bg-white p-5">
        <div className="flex items-center justify-between gap-7 pr-2">
          <div className="flex items-center gap-5">
            ...
            <div className="hidden h-[37px] items-center  justify-start gap-2 rounded-xl bg-darkslateblue-light px-3 md:flex">
              <img src="/search-icon.svg" alt="Search Icon" />
              <input
                className="w-64 border-none  bg-transparent px-2  py-1  font-poppins text-[14px] font-medium   leading-5 text-darkOliveGreen-dark placeholder-darkOliveGreen-200  focus:border-none focus:outline-none focus:ring-0 focus:ring-offset-0"
                placeholder="Search Here"
              />
            </div>
            <button className="flex h-[37px] w-[37px] items-center  justify-center rounded-xl bg-darkslateblue-light">
              <img src="/re-fresh.svg" alt="Refresh Icon" />
            </button>
          </div>
                ...
                <table className="min-w-full select-none ">
                  ...
                  <tbody className=" bg-white">
                    {patients.map((patient) => (
                      <tr key={patient.name} className="font-poppins">
                        <td className="">
                          <input
                            id="checkbox"
                            type="checkbox"
